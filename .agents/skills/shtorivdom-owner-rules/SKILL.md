...
- Оригиналы Figma (`mockups/landing-original*`) переводятся в статичный HTML на
...
- Переводы — `mockups/tailwind/site-N/index.html` (+ `app.js` с интерактивом), общие скрипты —
  `mockups/tailwind/shared/`, галереи — `mockups/tailwind/images/` и `icons/` (страницы в папках:
...
- **Проверка обязательна:** `check-site-N.mjs` (каждое взаимодействие, ошибки скрипта через
