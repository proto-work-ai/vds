...
export interface IMetaAttribute<T> extends IMetaAttributeBase<T> {
  type: MetaAttributeType | string | 'html';
}
...
export type ColumnAttributeTable<T = any> =
  | IMetaAttribute<T>
  | IMetaAttributeContextComponent<T>
  | IMetaAttributeContextTemplate<T>
  | IMetaAttributeContextElement<T>;
