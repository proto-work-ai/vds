...
(() => {
  ...
  const syncHeader = () =>
    header?.classList.toggle(
      'is-solid',
      solidAlways || scrollY > 60 || burger?.getAttribute('aria-expanded') === 'true',
    );
  syncHeader();
  ...
  $$('[data-nav]').forEach((a) => {
    ...
    if ((key === '' && page === '') || (key && page.startsWith(key)))
  ...
  addEventListener(
    'resize',
    () => innerWidth >= 1024 && burger?.getAttribute('aria-expanded') === 'true' && setMenu(false),
  );
  ...
  $$('.has-dropdown').forEach((d) =>
    d.addEventListener('keydown', (e) => e.key === 'Escape' && document.activeElement.blur()),
  );
  ...
  if (slider) {
    ...
    const fill = (i) => {
      ...
      if (nameEl) {
        ...
        nameEl.classList.remove('text-slate/60');
      }
      if (catEl) catEl.textContent = r.cat;
      if (avatar) {
        // Иллюстрация-силуэт, не фото человека: демо-отзывы вымышленные
        const hair = {
          long: 'M14 30c-3-8 0-18 10-18s13 10 10 18c-2-4-4-9-10-9s-8 5-10 9z',
          short: 'M15 20c0-6 4-9 9-9s9 3 9 9c-2-3-5-4-9-4s-7 1-9 4z',
          bob: 'M13 25c-1-9 3-14 11-14s12 5 11 14c-2-5-5-8-11-8s-9 3-11 8z',
        }[r.hair];
  ...
  const catalogData = (() => {
    try {
      return JSON.parse($('#calc-data')?.textContent || '[]');
    } catch {
      return [];
    }
  })();
  ...
  if (calc && catalogData.length) {
    ...
    const costOf = (c, w, h) => {
      ...
      return c.unit === 'м.пог.'
        ? (Math.ceil(((w / 100) * k + 0.2) * 10) / 10) * c.min
        : Math.max(0.5, Math.ceil((((w / 100) * h) / 100) * 100) / 100) * c.min;
    };

    const render = () => {
      const kind =
        curtains.find((c) => c.key === $('input[name="kind"]:checked', calc)?.value) ?? curtains[0];
      const byMeter = kind.unit === 'м.пог.';
      const w = Number(widthIn.value),
        h = Number(heightIn.value);
      ...
      if (!bad) {
        ...
        if (byMeter) {
          ...
          rows.push(
            ['Ширина × пышность', money(w) + ' см × ' + dec(k)],
      ...
      $('[data-calc-rows]', calc).innerHTML = rows
        .map(
          ([a, b]) =>
      ...
      if (hidden)
        hidden.value = calc.dataset.summary
          ? 'Расчёт с калькулятора — ' + calc.dataset.summary
          : '';
    };
  ...
  if (quiz && catalogData.length) {
    ...
    const next = $('[data-quiz-next]', quiz),
      back = $('[data-quiz-back]', quiz);
    const hint = $('[data-quiz-hint]', quiz),
      nav = $('[data-quiz-nav]', quiz);
    // Баллы разделам каталога за ответы
    const SCORES = {
      room: {
        bedroom: { 'blackout-curtains': 3, 'roman-blinds': 1 },
        living: { 'linen-curtains': 2, 'blackout-curtains': 1, 'curtain-rods': 1 },
        kids: { 'blackout-curtains': 2, 'roller-blinds': 2 },
        kitchen: { 'roman-blinds': 3, 'roller-blinds': 2, blinds: 1 },
        office: { blinds: 3, 'roller-blinds': 2 },
        other: { 'linen-curtains': 1, 'roller-blinds': 1 },
      },
      light: {
        dark: { 'blackout-curtains': 4, 'roller-blinds': 1 },
        soft: { 'linen-curtains': 3, 'roman-blinds': 1, 'pleated-blinds': 1 },
        privacy: { 'pleated-blinds': 2, 'roller-blinds': 2, blinds: 2 },
        decor: { 'roman-blinds': 2, 'linen-curtains': 2, 'curtain-rods': 1 },
      },
      window: {
        standard: {},
        panoramic: { 'linen-curtains': 2, 'blackout-curtains': 1, 'curtain-rods': 2 },
        attic: { 'pleated-blinds': 4, 'roller-blinds': 1 },
        door: { 'roller-blinds': 2, blinds: 2, 'pleated-blinds': 1 },
      },
      style: {
        classic: { 'roman-blinds': 2, 'blackout-curtains': 1, 'curtain-rods': 1 },
        modern: { 'roller-blinds': 2, blinds: 2, 'pleated-blinds': 1 },
    ...
    const renderResult = () => {
      ...
      const top = catalogData
        .filter((c) => score[c.key])
        .sort((a, b) => score[b.key] - score[a.key])
        .slice(0, 2);
      $('[data-quiz-cards]', quiz).innerHTML = top
        .map(
          (c) =>
            '<a href="' +
            c.href +
            '" class="work group relative block overflow-hidden rounded-[4px] bg-navy text-cream">' +
            '<div class="relative aspect-[4/3] overflow-hidden"><img src="' +
            c.image +
            '" alt="' +
            c.title +
        ...
        .join('');
      quiz.dataset.summary =
        steps
          .map(
            (st, i) =>
              st.querySelector('legend').textContent.trim() +
              ' ' +
              (answer(i)?.closest('label').querySelector('b').textContent ?? '—'),
          )
          .join('; ') +
        '. Рекомендация: ' +
    ...
    const show = () => {
      ...
      $('[data-quiz-bar]', quiz).style.width =
        (Math.min(current + (done ? 0 : 1), steps.length) / steps.length) * 100 + '%';
    ...
    $('[data-quiz-restart]', quiz).addEventListener('click', () => {
      $$('[data-quiz-step] input[type="radio"]', quiz).forEach((r) => (r.checked = false));
      current = 0;
      show();
    });
    show();
  ...
  if (document.currentScript?.dataset.emailV)
    emailUrl.search = '?v=' + document.currentScript.dataset.emailV; // версия против кеша хостинга
  ...
  const sendLead = ({ subject, html, text, clientEmail, client }) => {
    ...
    if (client)
      console.log(
        '[заявка] письмо клиенту ' + clientEmail + ':',
        client.subject,
        '\n' + client.text,
      );
  ...
  const previewLead = (subject, html) => {
    ...
    box.style.cssText =
      'position:fixed;inset:0;z-index:1000;background:rgb(8 22 41/.7);display:flex;align-items:center;justify-content:center;padding:16px';
    box.innerHTML =
  ...
  const submitLead = (form) => {
    ...
    if (isMock) {
      ...
      const lead = api.buildLeadEmail(kind, data, {
  ...
  $$('[data-lead-form]').forEach((form) => {
    ...
    form.addEventListener('submit', (e) => {
      ...
      Promise.resolve(submitLead(form))
        ...
        .catch((e) => {
          failBox().textContent =
            (e?.message || 'Не удалось отправить заявку') + '. Позвоните нам: +7 (915) 359-12-00';
          if (submit) {
  ...
  $$('[data-focus-form]').forEach((a) =>
    a.addEventListener('click', () =>
      setTimeout(() => $('#hero-form input[name="name"]')?.focus({ preventScroll: true }), 400),
    ),
  );
})();
