...
(() => {
  ...
  const PRELOAD = [
    ...
    '[transform:scale(1)]',
    '[transition:opacity_0.4s,transform_0.4s]',
    '[transition:height_0.3s,opacity_0.3s]',
  ...
  if (burger) {
    ...
    const open = () => {
      ...
      panel.className =
        '[background:rgb(26,_39,_68)] [overflow:hidden] [border-top:1px_solid_rgba(201,_168,_76,_0.2)] [opacity:0] [transition:height_0.3s,opacity_0.3s]';
  ...
  const setOpen = (target) => {
    for (const c of cards) {
      ...
      if (on && !c.list) {
        ...
        ul.className = '[overflow:hidden] [list-style:none] [padding:0px] [margin:0px_0px_16px] [opacity:0] [transition:height_0.3s,opacity_0.3s]';
  ...
  if (firstDot) {
    ...
    const MOVE = '[transition:opacity_0.4s,transform_0.4s]';
    let index = 0;
    let token = 0;
    const fill = (r) => {
  ...
  if (form) {
    ...
    form.addEventListener('submit', (e) => {
      ...
      done.className = '[text-align:center] [padding:40px_0px] [opacity:0] [transform:scale(0.95)] [transition:opacity_0.4s,transform_0.4s]';
      done.innerHTML = `
      ...
      reflow(done);
      swap(done, ['[opacity:0]', '[transform:scale(0.95)]'], ['[opacity:1]', '[transform:scale(1)]']);
    });
  }
})();
