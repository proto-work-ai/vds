import { Directive, Input, ElementRef, HostListener, AfterViewInit, ChangeDetectorRef, inject } from '@angular/core';
...
export class ParallaxDirective implements AfterViewInit {
  ...
  public ngAfterViewInit() {
    this.initialTop = this.eleRef.nativeElement.getBoundingClientRect().top + Math.floor(window.pageYOffset);
    this.getScreenSize();
    this.scrolling();
  ...
  public scrolling() {
    this.scroll$.pipe(throttleTime(5)).subscribe(() => {
      // Element position
      this.positionElement = (window.pageYOffset - this.initialTop) * this.ratio + this.screenHeightFactor;
      ...
      if (window.pageYOffset < window.innerHeight && window.pageYOffset >= 1) {
        ...
        this.parallaxStart =
          window.pageYOffset >= this.initialTop - this.eleRef.nativeElement.getBoundingClientRect().top * 2;
      } else {
      ...
      const parallaxEnd =
        window.pageYOffset <
        this.initialTop + window.innerHeight / 2 + this.eleRef.nativeElement.getBoundingClientRect().bottom;

      // Add styles to element
      if (this.parallaxStart && parallaxEnd) {
        if (this.diagonal) {
          this.eleRef.nativeElement.style = `transform:
            translateX(${-this.positionElement / 1.5}px) translateY(${this.positionElement}px);
