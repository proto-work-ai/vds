...
import { MetaAttribute, MetaEntity, MetaRecord, MetaValue } from '@prisma/client';
...
@Injectable()
export class MetaValueSevice {
  private prismaPagination = this.prisma.$extends(pagination());
  private get value() {
    return this.prisma.metaValue;
  }
  ...
  constructor(
    ...
    //@InjectRepository(MetaRecord)   private readonly recordRep: Repository<MetaRecord>,
  ...
  async getAll(params: {
    ...
    page: number,
      ...
      limit: 10,
      page: 1,
    }): Promise<{ data: MetaValue[], paginate: PageNumberPagination }> {
    console.log('params', params);
    return this.prismaPagination.metaValue
      .paginate()
      .withPages({
        ...params,
  ...
  async getById(name, parentId: string): Promise<MetaValue> {
    return this.value
      .findUnique({
        where: {
          parentId_name: { parentId, name }
        },
      })
      .then((value) => {
  ...
  async create({ name, parentId }: { name: string, parentId: string }, data: MetaValue): Promise<MetaValue> {
  ...
  async update({ name, parentId }: { name: string, parentId: string }, data: MetaValue): Promise<MetaValue> {
    return this.value.update({
      where: { parentId_name: { name, parentId } },
      data: {
        name: name,
        parentId: parentId,
        ...data,
  ...
  async deleteById({ name, parentId }: { name: string, parentId: string }, andRecords = true): Promise<MetaValue> {
    // if (andRecords) {
  ...
  // убираем дубли
  // protected async checkDoubles(record: MetaRecord) {
