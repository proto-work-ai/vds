...
export function ymInviteEvent(): void {
  ...
  if (ym) {
    ym(108545164, 'reachGoal', 'invite');
  }
}

@Directive({ selector: '[inviteModalClick]' })
export class InviteModalClick {
  private readonly destroyRef = inject(DestroyRef);
  private readonly dialogService = inject(TuiDialogService);

  @HostListener('click') protected onInvite(): void {
    this.dialogService
      .open<string>(new PolymorpheusComponent(InviteDesignerModal), {
        size: 'm',
...
@Component({
  templateUrl: 'invite-designer-modal.html',
  styleUrl: 'invite-designer-modal.scss',
  imports: [HlmButtonImports, FormImports],
  providers: [
    provideIcons({
      lucideDelete,
      lucideX,
...
export class InviteDesignerModal {
