...
if (!existsSync('mockups/site/index.html'))
  throw new Error('Нет mockups/site/index.html — сначала node tools/mockups/site-build.mjs');
...
const files = [
  ...
  ...walk('mockups/shared/fonts').map((f) => [
    f,
    posix(path.join('shared', path.relative('mockups/shared', f))),
  ]),
  ...
  ...walk('apps/shtorivdom-site/api')
    .filter(
      (f) =>
        !/lead-config\.php$/.test(f) &&
        !/[\\/]leads[\\/]/.test(f) &&
        !/send(-message)?\.php$/.test(f),
    )
...
await deploy(files, {
  ...
  uploadCommand: 'FTP_PASSWORD=… node tools/mockups/deploy-site-ftp.mjs --upload',
});
