...
const walk = (dir) =>
  readdirSync(dir).flatMap((n) => {
    ...
    if (statSync(p).isDirectory())
      return n === 'emails' || n === 'src' || n === 'assets' ? [] : walk(p);
...
const toTemplate = (html) =>
  html
    .replace(
      /<script type="application\/json" id="calc-data">([\s\S]*?)<\/script>/g,
      '<div id="calc-data" class="hidden" aria-hidden="true">$1</div>',
    )
    .replace(/\{/g, '&#123;')
    .replace(/\}/g, '&#125;')
    .replace(/@/g, '&#64;')
...
for (const file of walk(SITE).sort()) {
  const pagePath = path
    .relative(SITE, path.dirname(file))
    .split(path.sep)
    .join('/')
    .replace(/^\.?$/, '');
  ...
  const description = decode(
    src.match(/<meta name="description" content="([^"]*)"/)?.[1] ?? DESCRIPTIONS[route] ?? '',
  ).replace(/([а-яёa-z0-9])( Бесплатный выезд)/, '$1.$2');
...
`,
  ...
  const ogImage =
    src.match(/og:image" content="([^"]+)"/)?.[1] ??
    (firstImg
      ? SITE_URL + absolutize(`src="${firstImg}"`, route).slice(5, -1)
      : `${SITE_URL}/assets/img/hero.jpg`);
  ...
  if (route) {
    const items = crumbsNav
      ? [...absolutize(crumbsNav, route).matchAll(/<a href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/g)].map(
          (m) => ({ name: text(m[2]), url: SITE_URL + m[1] }),
        )
  ...
  const faq = [
    ...main.matchAll(
      /data-faq-q[^>]*>([\s\S]*?)<\/button>\s*<div[^>]*data-faq-a[^>]*>([\s\S]*?)<\/div>\s*(?=<\/div>|<\/li>|<\/article>)/g,
    ),
  ].map((m) => ({
    q: text(m[1]),
    a: text(m[2]),
  ...
  if (section && priceBlock) {
    const prices = [
      ...text(priceBlock).matchAll(/(\d[\d ]*\d|\d)\s*(?:–\s*(\d[\d ]*\d))?\s*₽/g),
    ].flatMap((m) => [m[1], m[2]].filter(Boolean).map((n) => Number(n.replace(/\s/g, ''))));
    if (prices.length) {
      ld.push({
        '@type': 'Product',
        name: h1,
  ...
  if (route === 'catalog/') {
    const cards = [
      ...absolutize(main, route).matchAll(/<a href="(\/catalog\/[^"]+\/)"[^>]*data-catalog-card/g),
    ].map((m) => m[1]);
    const list = cards.length
      ? cards
      : [
          ...new Set(
            [...absolutize(main, route).matchAll(/href="(\/catalog\/[a-z-]+\/)"/g)].map(
              (m) => m[1],
            ),
          ),
        ];
  ...
  routes.push({
    ...
    seo: {
      ...
      description,
      image: ogImage,
      jsonLd: ld.map((x) => ({ '@context': 'https://schema.org', ...x })),
    },
  });
}
...
for (const key of ['title', 'description']) {
  ...
  for (const r of routes) {
    if (seen.has(r.seo[key]))
      throw new Error(`Одинаковый ${key}: /${r.route} и /${seen.get(r.seo[key])}`);
    seen.set(r.seo[key], r.route);
  }
}

const q = (s) => `'${s.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}'`;
const seoLiteral = (route) => {
  const json = JSON.stringify(route.seo);
  const section = route.route.match(/^catalog\/([^/]+)\/$/)?.[1];
  return section
    ? json.replace(/"offers":\{[^{}]*\}/, `"offers":sitePriceOffer('${section}')`)
    : json;
};
writeFileSync(
...
${routes
  .map(
    ...
    pathMatch: 'full',
    data: { seo: ${seoLiteral(r)} satisfies SeoData },
...
writeFileSync(
  path.join(APP, 'src/app/layout/site-footer.html'),
  toTemplate(absolutize(footer, '')).trim() + '\n',
);
mkdirSync(path.join(APP, 'src/styles'), { recursive: true });
writeFileSync(
  path.join(APP, 'src/styles/theme.css'),
  readFileSync(path.join(SITE, 'src/partials/theme.css'), 'utf8'),
);
writeFileSync(
  path.join(APP, 'src/styles/catalog.css'),
  `/* Стили страниц каталога — mockups/site/src/partials/catalog-styles.html */\n${catalogCss}\n`,
);
...
cpSync(
  path.join(SITE, 'assets/logo/logo-1-email.png'),
  path.join(APP, 'public/assets/logo/logo-1-email.png'),
);
...
const missing = [];
for (const f of [
  ...routes.map((r) => path.join(pagesDir, r.slug, 'page.html')),
  path.join(APP, 'src/app/layout/site-footer.html'),
]) {
